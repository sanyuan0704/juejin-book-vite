...
function badExports(option, keys) {
  throw new Error(
    `'${option}' was specified for options.exports, but entry module has following exports: ${keys.join(
      ", "
    )}`
  );
}
...
export default class Bundle {
  ...
  fetchModule(importee, importer) {
    return Promise.resolve(
      importer === null ? importee : this.resolvePath(importee, importer)
    ).then((path) => {
      if (!path) {
        // external module
  ...
  generate(options = {}) {
    ...
    this.statements.forEach((statement) => {
      ...
      if (statement._leadingComments.length) {
        const commentBlock = statement._leadingComments
          .map((comment) => {
  ...
  getExportMode(exportMode) {
    ...
    if (!/(?:default|named|none)/.test(exportMode)) {
      throw new Error(
        `options.exports must be 'default', 'named', 'none', 'auto', or left unspecified (defaults to 'auto')`
      );
    }
